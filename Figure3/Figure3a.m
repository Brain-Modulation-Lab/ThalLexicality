...
data_1 = performance_tbl.production_latency(1:3:end);
SE_list_1 = performance_tbl.production_latency_SE(1:3:end);
% group level
mean_1 = mean(data_1); SEM_1 = std(data_1)/sqrt(length(data_1));

data_2 = performance_tbl.production_latency(2:3:end);
SE_list_2 = performance_tbl.production_latency_SE(2:3:end);
% group level
mean_2 = mean(data_2); SEM_2 = std(data_2)/sqrt(length(data_2));

data_3 = performance_tbl.production_latency(3:3:end);
SE_list_3 = performance_tbl.production_latency_SE(3:3:end);
% group level
...
y_label = ylabel(gca, 'Speech production latency (s)');
x_tick1 = text(1,-0.1,'All trials','HorizontalAlignment','center');
x_tick2 = text(2,-0.1,'Word trials','HorizontalAlignment','center');
x_tick3 = text(3,-0.1,'Nonword trials','HorizontalAlignment','center');

% plot statistical line
stat_h.line =plot([2,3],[2.25,2.25],...
'-k', 'linewidth',1);

stat_h.text =text(2.5,2.3,'**','FontSize',30,'HorizontalAlignment','center');
