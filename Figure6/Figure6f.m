...
speech_response_table(13:18,:) = [];Unit_Selectivity_table(13:18,:) = []; % 117 left
...
SigUp_list = find(speech_response_table.p_120 * height(speech_response_table) < 0.051 & speech_response_table.h_120 == -1);
...
%%%%%% 1: left s1 vs. left s2 vs. right sigup (could also use all contacts instead of sigup)
...
stat_h(1).line =plot([1,3],[2.5,2.5],...
'-k', 'linewidth',1);

stat_h(1).text =text(2,2.75,'ns','FontSize',20,'HorizontalAlignment','center');

y_label = ylabel(gca, 'Beta lexical selectivity index');
x_tick1 = text(1,-3.3,'Left session 1','HorizontalAlignment','center');
x_tick2 = text(2,-3.3,'Left session 2','HorizontalAlignment','center');
x_tick3 = text(3,-3.3,'Right','HorizontalAlignment','center');

saveas(gcf, 'Figure6f.fig');